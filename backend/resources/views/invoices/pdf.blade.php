...
<body>
    ...
    <div class="invoice-container">
        ...
        <table class="small" style="margin-bottom: 10px; width: 100%;">
            <tr>
                <td width="30%" class="text-left" style="font-size: 8px; font-weight: bold;">
                    @if($company->k2_recipient_code) <strong>K-2 Recipient Code:</strong>
                    {{ $company->k2_recipient_code }}<br> @endif
                    @if($company->gstin) <strong>GSTIN:</strong> {{ $company->gstin }}<br> @endif
                    @if($company->pan) <strong>PAN:</strong> {{ $company->pan }} @endif
                </td>
                <td width="30%" class="text-center">
                    <span style="font-size: 8px; font-weight: bold; white-space: nowrap;">|| Shri Banashankari Devi
                        Prasanna ||</span><br />
                    <div class="bold"
                        style="border: 1px solid #000; display: inline-block; padding: 2px 12px; font-size: 11px; margin-top: 5px;">
                        CASH / CREDIT BILL
                    </div>
                </td>
        ...
        <table style="margin: 0 auto; margin-bottom: 10px;">
            <tr>
                ...
                <td class="text-center" style="vertical-align: middle;">
                    <div class="bold" style="font-size: 20px; text-transform: uppercase; line-height: 1.1;">
                        {{ $company->company_name ?? 'Your Company' }}</div>
                    <div style="font-size: 10px; margin-top: 2px; font-weight: bold;">{{ $company->address ?? '' }}</div>
                </td>
            </tr>
        </table>
        ...
        <div style="margin-bottom: 15px;">
            ...
            <span
                style="font-size: 14px; font-weight: bold; border-bottom: 1px dotted #000; display: inline-block; width: 80%;">
        ...
        <div class="clearfix">
            ...
            <table class="totals-box">
                <tr>
                    ...
                    <td width="45%" class="text-right bold">{{ \App\Models\Invoice::formatIndianCurrency($subtotal) }}
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td class="text-right bold" style="border-bottom: none; font-size: 14px;">
                        {{ \App\Models\Invoice::formatIndianCurrency($grand_total) }}</td>
                </tr>
            </table>
        </div>
        ...
        <div style="float: right; width: 35%; text-align: right; margin-top: 20px;">
            @if($company->signature && file_exists(public_path('storage/' . $company->signature)))
                <img src="{{ public_path('storage/' . $company->signature) }}"
                    style="max-height: 50px; max-width: 150px;"><br>
            @else
